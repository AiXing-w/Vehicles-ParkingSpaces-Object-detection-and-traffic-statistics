...
opt = 0
if opt == 0:
    m_p = 'model_data/centernet_hourglass_coco.h5'
    c_p = 'model_data/coco_classes.txt'
    backb = 'hourglass'
    ns = False

else:
    m_p = 'model_data/centernet_resnet50_voc.h5'
    c_p = 'model_data/voc_classes.txt'
    backb = 'resnet50'
    ns = True
...
class CenterNet(object):
    ...
    _defaults = {
        ...
        # "backbone"          : 'resnet50',
        "model_path"        : m_p,  #'model_data/centernet_hourglass_coco.h5',
        "classes_path"      : c_p,  #'model_data/coco_classes.txt',
        "backbone"          : backb,  # 'hourglass',
    ...
    def detect_image(self, image):
        x_max_shape, y_max_shape = self.input_shape[0], self.input_shape[1]
        #---------------------------------------------------------#
        ...
        top_objs = det_label[top_indices]
        obj1sum = len(top_objs[top_objs == 0])
        obj2sum = len(top_objs[top_objs == 1])

        top_label_indices = top_objs.tolist()
        ...
        for i, c in enumerate(top_label_indices):
            ...
            x_mid = (left + right) / 2
            y_mid = (bottom + top) / 2
            if predicted_class == "space-empty":
                if x_mid <= x_max_shape / 2 and y_mid <= y_max_shape / 2:
                    cnt1 += 1
                elif x_mid >= x_max_shape / 2 and y_mid <= y_max_shape / 2:
                    cnt2 += 1
                elif x_mid <= x_max_shape / 2 and y_mid >= y_max_shape / 2:
                    cnt3 += 1
                else:
        ...
        return image, obj1sum, obj2sum, self.colors[0], self.colors[1], max_cnt
